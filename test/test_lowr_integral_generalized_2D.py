...
class Test_M(unittest.TestCase):
    ...
    def setUp(self):
         ...
         l = tf.Variable([0.2, 0.5], dtype=default_float(), name='l')
         kernel = gfk.SquaredExponential(variance= v, lengthscales= l)
         lrgp = LowRankGeneralizedStationary(kernel, beta0 = None, space = Space([-1,1]), n_components =  10, n_dimension = 2, random_state = rng)
    ...
    def test_cc_vs_cc_Block(self):
       ...
       # #### cos_cos block
       print("cc_vs_cc_block[%i,%i]" % (b0,b0))
       ...
       print("cc_vs_cc_block[%i,%i]" % (b1,b1))
    ...
    def test_ss_vs_cc_block(self):
       ...
       print("ss_vs_cc_block[%i,%i]" % (r[0],c[0]))
       ...
       print("ss_vs_cc_block[%i,%i]" % (r[1],c[1]))
